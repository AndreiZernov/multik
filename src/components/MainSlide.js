  ...
  useEffect(() => {
    if (window.innerWidth > 768 && name !== "video") {
      let tlHeroScroll1 = new TimelineMax({ paused: true })
        .to(
          `.main-${name}1-img, .main-${name}3-img, .main-${name}4-img, .main-${name}5-img`,
          1,
          {}
        )
        .to(
      ...
      let tlHeroScroll3 = new TimelineMax({ paused: true }).to(
        slides.current,
        1,
      ...
      >
        ...
        {name !== "video" ? (
          ...
          >
            {[1, 2, 3, 4, 5].map((num) => (
              <Link
                key={num}
                to={name}
                style={{ backgroundImage: `url(${data[num - 1]})` }}
                className={`main-${name}${num}-img`}
              ></Link>
            ))}
          </div>
        ) : (
          <video muted controls>
            <source
              poster={require("../assets/mltl_white.png")}
              src={data[0]}
              type="video/mp4"
            />
            <source
